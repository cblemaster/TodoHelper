...
{
    public static string IsNotUniqueErrorMessage(string entityName, string attributeName, string value) =>
        $"{entityName} with {attributeName} {value} already exixts.";
    ...
    public static string UnknownErrorMessage(string action) => $"An unknown error occurred when {action}.";
}
